...
# ╔═╡ d89f0f5a-213a-11f0-0c84-a55aafe692ed
begin
	using Plots
	using LaTeXStrings
...
# ╔═╡ 3d5cb39e-a484-4da7-a11f-4e37bfb2d987
using CairoMakie

# ╔═╡ 1d68a299-509b-4e69-9a83-a0fe65d227c1
...
# ╔═╡ 17d09f79-4194-426c-86b5-4f88f2992f82
function sol_lorenz(r)
    u0 = [1.0, 0.0, 0.0]
    tspan = (0.0,100.0)
    p = (r=r,)
...
# ╔═╡ 68412cfc-5b8b-4cf4-80d1-0008b2205ef2
function plot_shi(r)
	sol = sol_lorenz(r)
	savefig(plot(sol, idxs=(0,1), label="x(t)", ylabel="x", lw=1), "$(r)_x.png")
	savefig(plot(sol, idxs=(0,2), label="y(t)", ylabel="y", lw=1), "$(r)_y.png")
	savefig(plot(sol, idxs=(1,2), label="", xlabel="x", ylabel="y", lw=1), "$(r)_yvx.png")
	savefig(plot(sol, idxs=(1,3), label="", xlabel="x", ylabel="z", lw=1), "$(r)_zvx.png")
end

# ╔═╡ 9fd60be9-4f1e-4347-9514-796d0d8930e5
# ╠═╡ disabled = true
#=╠═╡
for r in [10.0, 24.5, 126.5]
	plot_shi(r)
end
  ╠═╡ =#

# ╔═╡ 42d41791-7576-4bee-bae8-f35ae241bf19
function lienard!(du,u,p,t)
    μ = p.μ
    du[1] = u[2]
    du[2] = (μ - u[1]^2)*u[2] - u[1]
end

# ╔═╡ 3e0ca19f-0ff2-4413-8417-186c5f5bf255
function sol_lienard(μ)
    u0 = [0.0, 1.0]
    tspan = (0.0, 40.0)
    p = (μ=μ,)
    prob = ODEProblem(lienard!, u0, tspan, p)
    sol = solve(prob, saveat=0.02)

    return sol
end

# ╔═╡ a56f127a-1456-43eb-85cc-a2621b0072bd
function plot_lienard(μ)
	f = Figure(size = (800, 800), xlabel=L"x", ylabel=L"y")
	Axis(f[1, 1])
	
	xs = LinRange(-3, 3, 20)
	ys = LinRange(-5.5, 5.5, 20)
	
	us = [y for x in xs, y in ys]
	vs = [(μ-x^2)*y-x for x in xs, y in ys]
	strength = vec(@. sqrt(us^ 2 + vs^ 2))
	
	arrows!(xs, ys, us, vs, arrowsize = 10, lengthscale = 0.05,
	    arrowcolor = strength, linecolor = strength)
	
	sol = sol_lienard(μ)
	CairoMakie.lines!(sol[1, :], sol[2, :])

	save("lienard-$μ.png", f)
	return f
end

# ╔═╡ ac752632-a9de-437e-bdc8-6a9d5cfd1bc2
# ╠═╡ show_logs = false
for μ in [-0.5, -0.1, 0, 0.1, 0.5, 2]
	plot_lienard(μ)
end
...
manifest_format = "2.0"
project_hash = "48b4b918e3ec65b7f8dd5c2388105faa278b2c36"
