...
# ╔═╡ 84d3b090-856d-11ef-0471-f991cc8bbdba
begin
...
# ╔═╡ 6d1e7f51-7299-4ca0-8c49-91f64df51d7e
# ╠═╡ disabled = true
#=╠═╡
function UnifOsc(θ, p, t)
	dθ = p
	return dθ
end
  ╠═╡ =#

# ╔═╡ b5ec5a84-ddab-4543-904b-303e2f7edfa3
# ╠═╡ disabled = true
#=╠═╡
function Sol(prob, p; start=0, t=2, step_size=0.01)
	θ₀ = start
	tspan = (0.0, Float64(t))
	osc_prob = ODEProblem(prob, θ₀, tspan, p)
	osc_sol = solve(osc_prob, saveat=step_size)
	
	return osc_sol
end
  ╠═╡ =#

# ╔═╡ 624306e5-8929-4896-9bc1-34c43fedba92
#=╠═╡
unif_osc_sol = Sol(UnifOsc, 2pi)
  ╠═╡ =#

# ╔═╡ 761d7973-2e10-4f5b-a6fe-be97cd87205d
#=╠═╡
Plots.plot(unif_osc_sol)
  ╠═╡ =#

# ╔═╡ 3246d9e2-0224-47e8-a74a-910f5e7c74d1
#=╠═╡
length(unif_osc_sol.t)
  ╠═╡ =#

# ╔═╡ 7d082f87-1c32-4f58-aa84-1e0edea43e6d
#=╠═╡
begin
	osc1 = Sol(UnifOsc, 2pi)
	osc2 = Sol(UnifOsc, pi/2)
end
  ╠═╡ =#

# ╔═╡ 8b16182b-b769-4684-8019-43040b29fea4
#=╠═╡
begin
	anim = Animation()
	
	for x in 1:length(unif_osc_sol.t)
		plot((range(0, 2pi, 100), 1), linewidth=2, linecolor=:gray, leg=false, proj=:polar, grid=false, showaxis=false, ticks=[1], ylims=(0, 1.1))
		scatter!([(osc1[x], 1), (osc2[x], 1)])
	    frame(anim)
	end
	gif(anim, "./outputs/2_unif_osc.gif", fps=30)
end
  ╠═╡ =#

# ╔═╡ b05e9e35-7036-444d-a61f-2faa576ed1a8
# ╠═╡ disabled = true
#=╠═╡
function UnifOscs!(dθ, θ, p, t, n)
	for i in 1:n
		dθ[i] = p * i
	end
end
...
# ╔═╡ 34763829-abe1-4a2a-b40c-ca34a04ec5bd
# ╠═╡ disabled = true
#=╠═╡
function Sols(prob, p, n; start=0, t=2, step_size=0.01)
	
	θ₀ = fill(start, n)
	tspan = (0.0, Float64(t))
	f = (dθ, θ, p, t) -> prob(dθ, θ, p, t, n)
	osc_prob = ODEProblem(f, θ₀, tspan, p)
	osc_sol = solve(osc_prob, saveat=step_size)
...
# ╔═╡ 2e8922ed-587c-487a-bf8a-4a08d904a421
#=╠═╡
begin
	phase_diff = pi/2
	num_unif_osc = 3
	unif_osc_sols = Sols(UnifOscs!, phase_diff, num_unif_osc)
end
  ╠═╡ =#

# ╔═╡ 7e0d50a1-1351-47ee-b3af-abd5fcf719e3
...
# ╔═╡ f8a1c948-2930-41b3-9b97-d16d5247a21e
function PlotOscillators(sol, fname)
	N = size(sol)[1]
	...
	anim = @animate for t in 1:length(sol)
		...
		plot!([(0, 0), centroid], marker=:circle, markersize=[0, 5], line=:solid)
	end
	
	return gif(anim, fname, fps=30)
end

# ╔═╡ c41b89a3-c523-41e9-825d-85d0c82ceadf
#=╠═╡
PlotOscillators(unif_osc_sols, "./outputs/$(num_unif_osc)_unif_osc.gif")
  ╠═╡ =#

# ╔═╡ 257bd52f-e325-4e1a-9149-a568186e2d5f
# ╠═╡ disabled = true
#=╠═╡
function NonUnifOsc(θ, p, t)
	dθ = p[1] - p[2] * sin(θ)
	return dθ
end
  ╠═╡ =#

# ╔═╡ 60afcdad-4699-4854-9878-132d478edc8f
#=╠═╡
begin
	non_unif_osc_1 = Sol(NonUnifOsc, [2.1, 2]; t=100, step_size=0.1)
	# osc2_test = Sol(UnifOsc, pi/2)
end
  ╠═╡ =#

# ╔═╡ 13ae51fb-c4b6-4e85-b2b7-4983855bc0c2
#=╠═╡
begin
	anim2 = Animation()
	
	for x in 1:length(non_unif_osc_1.t)
		plot((range(0, 2pi, 100), 1), linewidth=2, linecolor=:gray, leg=false, proj=:polar, grid=false, showaxis=false, ticks=[1], ylims=(0, 1.1))
		scatter!((non_unif_osc_1[x], 1))
	    frame(anim2)
	end
	gif(anim2, "./outputs/non_unif_osc.gif", fps=30)
end
  ╠═╡ =#

# ╔═╡ cf1d16d6-e92b-4c10-adad-4064bc031499
#=╠═╡
plot(non_unif_osc_1)
  ╠═╡ =#
...
function SolKuramoto(N, K; ω=randn(N)*2, θ₀=rand(Float64,N)*2π, t=2, step_size=0.01)
	...
	sol = solve(prob, DP5(), saveat=step_size)
	# sol = solve(prob, DP5(), save_start=false, save_everystep=false)
...
begin
	varK_N = 500 #800
	varK_step_size = 0.1
	varK_ω = randn(varK_N) #vcat(randn(varK_N÷2) .+ 2, randn(varK_N÷2) .- 2)
	varK_θ₀ = rand(Float64, varK_N) * 2π
	varK_plot = plot(leg=false)

	r_inf = []

	varK_fwd = collect(0:0.5:8)
	varK_rev = reverse(varK_fwd)
	varK_len = length(varK_fwd)

	@progress for K in vcat(varK_fwd, varK_rev)
		coherences = []
		varK_kuramoto_sol = SolKuramoto(varK_N, K; ω=varK_ω, θ₀=varK_θ₀, t=25, step_size=varK_step_size)

		for t in 1:length(varK_kuramoto_sol)
			phases, centroid = GetPhaseData(varK_kuramoto_sol, t)
			push!(coherences, centroid[2])
		end

		plot!(coherences, line_z=K, c=:viridis)
		push!(r_inf, coherences[end]) # mean(last(coherences, 50))

		global varK_θ₀ = varK_kuramoto_sol[end]
	end
...
begin
	varK_N = 100 #800
	varK_step_size = 0.1
	varK_ω = randn(varK_N) * 2
	varK_θ₀ = rand(Float64, varK_N) * 2π
	varK_plot = plot(leg=false)

	r_inf = []

	varK = range(start=0, stop=10, step=0.2)

	@progress for K in varK
		coherences = []
		varK_kuramoto_sol = SolKuramoto(varK_N, K; ω=varK_ω, θ₀=varK_θ₀, t=20, step_size=varK_step_size)
...
manifest_format = "2.0"
project_hash = "07840ff89bfb12031faee39290c8edf7031da02e"
